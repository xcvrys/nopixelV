<script lang="ts">
	...
	function handleRotationResult(res: {
...
<div
	class="flex-1 flex flex-col items-center justify-center p-6 max-w-4xl mx-auto w-full min-h-[calc(100vh-2rem)]"
>
	<!-- Header title -->
	<div class="text-center mb-6">
		<div
			class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-neutral-900 border border-neutral-800 text-neutral-300 text-xs font-semibold uppercase tracking-wider mb-2"
		>
		...
		<p class="text-neutral-400 text-sm mt-1 max-w-lg">
			Rotate the dial with mouse drag or <kbd
				class="px-1.5 py-0.5 rounded bg-neutral-900 text-white font-mono text-xs border border-neutral-800"
				>A</kbd
			>/<kbd
				class="px-1.5 py-0.5 rounded bg-neutral-900 text-white font-mono text-xs border border-neutral-800"
				>D</kbd
			>
			keys. Listen carefully for the deep acoustic notch clunk, then press
			<kbd
				class="px-1.5 py-0.5 rounded bg-neutral-900 text-white font-mono text-xs border border-neutral-800"
				>Space</kbd
			> to unlock the stage.
	...
	<div
		class="relative w-full max-w-lg bg-neutral-950 border border-neutral-800 rounded-2xl p-8 flex flex-col items-center {failedAttemptShake
			? 'animate-bounce'
			: ''}"
	>
		<!-- Stages Counter -->
		<div class="flex items-center justify-center gap-2.5 w-full mb-6">
			{#each code as secretNum, i}
				<div
					class="flex-1 flex flex-col items-center p-2.5 rounded-lg border transition-all {i <
					currentStage
		...
		<div class="relative flex items-center justify-center my-4">
			...
			<div
				class="absolute -top-3 z-30 flex flex-col items-center pointer-events-none"
			>
				<div class="w-1.5 h-3.5 bg-white rounded-b"></div>
				<div
					class="w-0 h-0 border-l-[4px] border-l-transparent border-r-[4px] border-r-transparent border-t-[6px] border-t-white"
				></div>
			</div>
			...
			>
				...
				<div
					class="relative w-28 h-28 md:w-32 md:h-32 rounded-full bg-neutral-950 border border-neutral-800 flex items-center justify-center pointer-events-none"
				>
					<!-- Grip Ridges -->
					<div
						class="w-20 h-20 rounded-full border border-neutral-850 flex items-center justify-center"
					>
						<div
							class="w-12 h-12 rounded-full bg-black border border-neutral-800 flex items-center justify-center"
						>
		...
		<div
			class="flex items-center justify-between w-full mt-4 pt-4 border-t border-neutral-900"
		>
			<div>
				<span
					class="text-[10px] font-semibold text-neutral-500 uppercase tracking-wider block"
					>Dial Reading</span
				>
				<span class="font-mono font-bold text-2xl text-white"
					>{currentNumber}</span
				>
			</div>
		...
		{#if status === "won"}
			<div
				class="w-full mt-6 p-4 rounded-xl bg-black border border-neutral-800 text-center animate-in fade-in duration-150"
			>
				<div
					class="flex items-center justify-center gap-2 text-white font-bold mb-1"
				>
					<Trophy class="w-4 h-4 text-white" />
					<span>Safe Successfully Cracked!</span>
				</div>
				<p class="text-neutral-400 text-xs">
					Combination <span class="text-white font-mono font-bold"
						>{code.join(" - ")}</span
					>
					unlocked in
	...
	<div
		class="w-full max-w-lg mt-6 bg-neutral-950 border border-neutral-900 rounded-xl p-5"
	>
		<div class="flex items-center justify-between mb-4">
			<div
				class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-neutral-400"
			>
