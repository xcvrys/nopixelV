...
{#if snapshot.mode === "progressive"}
  <div class="flex items-center gap-3 text-xs font-bold italic uppercase tracking-wider">
    ...
    <div class="flex items-center gap-1.5">
      {#each [1, 2, 3] as stage}
        ...
        {@const isFailed = stage === snapshot.currentStage && snapshot.status === "failed"}
        {@const isActive = stage === snapshot.currentStage && !isPassed && !isFailed}
        <div
    ...
    <span class="text-[11px] font-mono text-neutral-400">
      ({snapshot.currentStage === 1 ? "EASY" : snapshot.currentStage === 2 ? "MEDIUM" : "HARD"})
    </span>
  </div>
{:else if snapshot.mode === "maxing"}
  <div class="flex items-center gap-4 text-xs font-bold italic uppercase tracking-wider">
    <div class="flex items-center gap-1.5">
      <span class="text-neutral-500">LEVEL</span>
      <span class="text-sm font-mono tabular-nums text-white">{snapshot.currentLevel}</span>
    </div>
    ...
    <div class="group relative flex cursor-pointer items-center gap-1.5">
      ...
      <span class="text-sm font-mono tabular-nums text-amber-400">{snapshot.bestStreak}</span>
      <div
      ...
      >
        ...
        <span class="font-bold tabular-nums text-white">
          {snapshot.bestStreakTime > 0 ? `${snapshot.bestStreakTime.toFixed(1)}s` : "—"}
        </span>
      </div>
    </div>
  </div>
{:else if snapshot.mode === "single"}
  <div class="flex items-center gap-1 border border-neutral-900 bg-neutral-950 p-0.5">
