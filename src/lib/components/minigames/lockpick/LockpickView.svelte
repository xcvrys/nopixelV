<script lang="ts">
  ...
  let dashOffset = $derived(
    circumference - (circumference * Math.min(100, Math.max(0, lockpick.snapshot.progress))) / 100,
  );

  let countdownColor = $derived.by(() => {
    const ratio = Math.max(0, Math.min(1, lockpick.snapshot.stageTimeLeft / lockpick.stageTimeout));
    const hue = Math.round(ratio * 32);
    return `hsl(${hue}, 95%, 55%)`;
  });
  function handleSurfaceKeydown(event: KeyboardEvent) {
    if (lockpick.inputMode !== "touch" || (event.key !== "Enter" && event.key !== " ")) {
      return;
    }
...
>
  ...
  >
    ...
    <LockpickStageTracker
      ...
      onDifficultyChange={(difficulty) => lockpick.setSingleDifficulty(difficulty)}
    />
  </div>
  ...
  >
    ...
    {:else if lockpick.snapshot.status === "idle"}
      <span class="text-xs md:text-sm font-bold italic tracking-wider text-neutral-500 uppercase">
    ...
    {:else if lockpick.snapshot.status === "failed"}
      ...
      >
        {lockpick.snapshot.failReason === "timeout" ? "TIME EXPIRED" : "LOCKPICK SNAPPED"}
      </span>
    {:else if lockpick.snapshot.status === "won"}
      <span
