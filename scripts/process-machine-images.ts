...
const sourceDirectory = join(projectRoot, "assets/machines/source");
const outputDirectory = join(projectRoot, "static/images/machines");
const temporaryDirectory = join(projectRoot, ".tmp/machine-images");
...
async function processImage(filename: string): Promise<void> {
  ...
  console.log(`${filename} -> static/images/machines/${slug}.webp`);
}
...
console.log(`Processed ${sourceFiles.length} machine image${sourceFiles.length === 1 ? "" : "s"}.`);
